...
from .prompt_template import detect_language, build_system_prompt
...
def ask_groq_general(user_message: str, conversation_history: list = None, language: str = 'vi') -> str:
    """
    ...
    if language == 'en':
        system_instruction = """You are a friendly and helpful AI assistant - like ChatGPT.
Answer questions naturally, accurately and helpfully.

STYLE GUIDELINES:
- Add appropriate emoji/icons at the beginning or middle of response
- Examples: 🌤️ for weather, 🔢 for math, 💡 for suggestions, ❓ for questions
- Clear formatting with line breaks to separate paragraphs
- Friendly, supportive, and helpful tone

If you don't know, say: ❌ I don't know, but I can help you find out"""
    else:
...
def ask_groq_stream_general(user_message: str, conversation_history: list = None, language: str = 'vi'):
    """
...
def ask_groq(user_message: str, data_context: dict, conversation_history: list = None, is_detailed: bool = False, language: str = 'vi') -> str:
    """
    ...
    # ── Build data context with language awareness ────
    if language == 'en':
        dominant_en = {
            "quantity": "sales quantity decreased",
            "price":    "selling price decreased",
            "both":     "both sales quantity and price decreased",
        }.get(data_context.get("dominant", "both"), "unknown")
        
        data_lines = ""
        if data_context:
            data_lines = (
                f"- Current month: {data_context.get('cur_month', 'N/A')}\n"
                f"- Revenue change: {data_context.get('chg_pct', 0)}% "
                f"compared to {data_context.get('prev_month', 'previous month')}\n"
                f"- Current revenue: {data_context.get('cur_rev', 'N/A')}\n"
                f"- Previous revenue: {data_context.get('prev_rev', 'N/A')}\n"
                f"- Most affected product: {data_context.get('worst_product', 'N/A')} "
                f"({data_context.get('prod_chg', 0)}%)\n"
                f"- Worst performing channel: {data_context.get('worst_channel', 'N/A')} "
                f"({data_context.get('ch_chg', 0)}%)\n"
                f"- Main cause: {dominant_en}\n"
            )
            if "product_breakdown" in data_context:
                data_lines += "\nProduct details (Top 3 most impactful):\n"
                for prod in data_context["product_breakdown"][:3]:
                    data_lines += f"  + {prod['product']}: {prod['chg_pct']}% (contributing {prod['impact_pct']}% to revenue decline)\n"
            if "channel_breakdown" in data_context:
                data_lines += "\nChannel details:\n"
                for ch in data_context["channel_breakdown"]:
                    data_lines += f"  + {ch['channel']}: {ch['chg_pct']}% (contributing {ch['impact_pct']}%)\n"
            if "region_breakdown" in data_context:
                data_lines += "\nRegion details:\n"
                for reg in data_context["region_breakdown"]:
                    data_lines += f"  + {reg['region']}: {reg['chg_pct']}% (contributing {reg['impact_pct']}%)\n"
        
        data_block = f"\n[ACTUAL DATA FROM sales.csv]\n{data_lines}[END OF DATA]\n" if data_lines else ""
        word_limit = "800-1200 words" if is_detailed else "500-800 words"
        
        system_instruction = f"""You are Revenue AI - a professional business analyst.
{data_block}

⚠️ ABSOLUTE RULES (MANDATORY):

1. **ONLY use provided data** - NO guessing, NO making up numbers
   - If insufficient data → respond: "Insufficient data to determine accurately"
   - Every number must come from [ACTUAL DATA FROM sales.csv]
   - WRONG: "Maybe due to market demand" - NOT IN DATA
   - CORRECT: "Laptop decreased -87.5%, Phone decreased -80.2%" - IN DATA

2. **NO guessing about causes**
   - Only state data, no speculation
   - ✓ "Quantity decreased 15%, price decreased 20%"
   - ✗ "Possibly due to competition"

3. **MUST cite specific numbers**
   - ✓ "Revenue: $13,997,655,537, decreased -81.2%"
   - ✗ "Revenue decreased significantly"

4. **MANDATORY HTML structure** (NO MARKDOWN):
   <strong>📊 RESULTS</strong><br>
   Specific numbers from data<br><br>
   <strong>🔍 PRIMARY CAUSE</strong><br>
   Analysis of quantity vs price from data<br><br>
   <strong>📈 DETAILS</strong><br>
   Top 3 products: - Laptop: -87.5%<br> - Phone: -80.2%<br> - Tablet: -80.6%<br><br>
   <strong>💡 RECOMMENDATIONS</strong><br>
   Based on data

5. **MANDATORY HTML TAGS:**
   - ✓ Use: <strong>TEXT</strong>, <br>, <em>TEXT</em>
   - ✗ DO NOT use: **TEXT**, _TEXT_, newlines
   - ✓ EXAMPLE: <strong>Laptop</strong> decreased <strong>-87.5%</strong><br>
   - ✗ WRONG: **Laptop** decreased **-87.5%**

6. **Word count**: {word_limit}
7. **Tone**: Professional, data-driven, specific"""
...
def ask_groq_stream(user_message: str, data_context: dict, conversation_history: list = None, is_detailed: bool = False, language: str = 'vi'):
    """
...
def ask_gemini(user_message: str, data_context: dict, conversation_history: list = None, is_detailed: bool = False, language: str = 'vi') -> str:
    """
...
1. **ONLY use provided data** - NO guessing, NO making up numbers
   - If no data available → answer: "Insufficient data to determine accurately"
   - Every number must come from [DATA FROM sales.csv]

2. **DO NOT guess root causes**
   - Only state possibilities based on data
   - Example: "Data shows quantity down 15%, price down 20%" (CORRECT)
   - Never say: "Possibly due to market demand decline" (WRONG)

3. **MUST cite specific numbers**
   - Always write: "March revenue is 93 million, down 31.2% vs February"
   - Never write: "Revenue declined significantly"

4. **MANDATORY response structure**:
   <strong>📊 SUMMARY</strong><br>
   [Main numbers from data]<br><br>
   <strong>🔍 ROOT CAUSE</strong><br>
   [Analyze quantity vs price based ONLY on data]<br><br>
   <strong>📈 DETAILS</strong><br>
   [Top 3 affected products/channels + impact %]<br><br>
   <strong>💡 RECOMMENDATIONS</strong><br>
   [Based on provided data]

5. **Length**: {word_limit}
6. **HTML**: <br>, <strong>, <em> - NO markdown
7. **Tone**: Data expert, confident, logical"""
    else:  # Vietnamese
        system_instruction = f"""Bạn là Revenue AI - nhà phân tích doanh thu chuyên nghiệp.
{data_block}
